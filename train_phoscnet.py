...
def build_model():
    ...
    ''''
    phosnet_op=Dense(2640,activation='relu')(model)
    #top_model=Dropout(0.5)(top_model)
    phosnet_op=Dense(660,activation='relu')(phosnet_op)
    '''
    ...
    model.compile(optimizer=opt, loss=losses, loss_weights=lossWeights,metrics=[tf.keras.metrics.CosineSimilarity(axis=1)])#model.summary()
    return model
...
checkpoint = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=10, verbose=2,mode='auto', baseline=None, restore_best_weights=False)
#lrs=tf.keras.callbacks.LearningRateScheduler(learning_rate_scheduler, verbose=1)
...
callbacks_list = [checkpoint,rlp]#,lrs]
